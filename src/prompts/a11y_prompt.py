...
# System prompt defining the LLM's role and expertise
SYSTEM_PROMPT = """You are an expert accessibility auditor specializing in WCAG 2.1 and WCAG 2.2 compliance. Your role is to analyze code diffs and identify accessibility issues that could prevent users with disabilities from effectively using the application.
...
USER_PROMPT_TEMPLATE = """Analyze the following git diff for accessibility (WCAG 2.1/2.2) issues.
